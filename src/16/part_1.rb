...
limits = validations.split("\n").map{|v| v.split(":")[1].strip}
min_limit = limits.map{|l| l.split("-")[0]}.map(&:to_i).min
max_limit = limits.map{|l| l.split("-").last}.map(&:to_i).max

input = File.read("nearby.txt").split("\n")

invalid = []
input.each do |ticket| 
  ticket.split(",").map(&:to_i).each do |t| 
    if t < min_limit || t > max_limit then invalid << t end
  end
end
